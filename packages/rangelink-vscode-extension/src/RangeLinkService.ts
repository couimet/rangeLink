...
import { ActiveSelections, MessageCode, type QuickPickBindResult } from './types';
import {
  formatMessage,
 ...
 *
 * **Architecture note (Issue #255):**
 * This service orchestrates picker → bind flows, eliminating the circular
 * dependency between DestinationPickerCommand and PasteDestinationManager.
 * The picker command returns user selection, this service calls bind().
 */
export class RangeLinkService {
  ...
  /**
   * Shows destination picker and binds to selected destination for paste flows.
   *
    ...
    switch (result.outcome) {
      ...
      case 'selected': {
        ...
        if (!bindResult.success) {
          this.logger.info(
            { ...logCtx, bindResult: bindResult.toJSON() },
