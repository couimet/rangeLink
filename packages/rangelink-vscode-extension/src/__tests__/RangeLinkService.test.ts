...
let mockActiveTerminal: vscode.Terminal;
...
describe('RangeLinkService', () => {
  ...
  describe('copyToClipboardAndDestination', () => {
    ...
    describe('when no destination is bound', () => {
      it('copies to clipboard with status message, does not send to destination or warn', async () => {
      ...
      it('uses link type name in status message for Portable RangeLink', async () => {
    ...
    describe('when destination is bound and paste succeeds', () => {
      ...
      it('copies to clipboard then sends to destination in order, no status bar or warnings', async () => {
        const link = 'src/auth.ts#L10-L20';
        const formattedLink = createMockFormattedLink(link);
    ...
    describe('when destination is bound but paste fails', () => {
      ...
      it('copies to clipboard and attempts send to destination (paste fails silently)', async () => {
        const link = 'src/auth.ts#L10-L20';
        const formattedLink = createMockFormattedLink(link);
      ...
      it('passes correct basicStatusMessage for Portable RangeLink', async () => {
    ...
    describe('when destination is bound but content not eligible', () => {
      ...
      it('copies to clipboard only, skips destination, logs eligibility skip', async () => {
        const link = 'src/auth.ts#L10-L20';
        const formattedLink = createMockFormattedLink(link);
  ...
  describe('pasteTerminalSelectionToDestination', () => {
    ...
    it('writes terminal text to clipboard via copyAndSendToDestination', async () => {
      await service.pasteTerminalSelectionToDestination();

      expect(mockClipboard.writeText).toHaveBeenCalledWith('terminal selected text');
    });

    describe('when no active terminal', () => {
      beforeEach(() => {
        mockShowErrorMessage = jest.fn().mockResolvedValue(undefined);
        mockVscodeAdapter = createMockVscodeAdapter({
          envOptions: { clipboard: mockClipboard },
    ...
    describe('when clipboard is empty after copy', () => {
      beforeEach(() => {
        mockClipboard = createMockClipboard({
    ...
    describe('when no destination bound', () => {
      ...
      it('shows picker and sends after binding', async () => {
        const mockTerminal = createMockTerminal();
        const mockDestination = createMockTerminalPasteDestination({ displayName: 'Terminal' });
        mockPickerCommand.pick.mockResolvedValue({
          outcome: 'selected',
    ...
    describe('terminal self-paste detection', () => {
      ...
      it('sends when bound destination is a text editor', async () => {
