...
describe('showTerminalPicker', () => {
  ...
  describe('non-bindable terminal selection', () => {
    it('returns undefined and does NOT invoke onSelected when picking a non-bindable terminal', async () => {
      const terminal = createMockTerminal({ name: 'jest' });
      const ptyItem = createMockTerminalQuickPickItem(
        terminal,
        false,
        undefined,
        'extension-managed',
      );
      const items: TerminalBindableQuickPickItem[] = [ptyItem];
      const quickPickProvider = createMockQuickPickProvider();
      quickPickProvider.showQuickPick.mockResolvedValueOnce(reformattedItem(ptyItem));
      const onSelected = jest.fn(identityCallback);
      const logger = createMockLogger();

      const result = await showTerminalPicker(
        items,
        quickPickProvider,
        createHandlers(onSelected),
        logger,
      );
    ...
    it('routes through onDismissed when provided and a non-bindable terminal is picked', async () => {
      const terminal = createMockTerminal({ name: 'jest' });
      const ptyItem = createMockTerminalQuickPickItem(
        terminal,
        false,
      ...
      const quickPickProvider = createMockQuickPickProvider();
      quickPickProvider.showQuickPick.mockResolvedValueOnce(reformattedItem(ptyItem));
      const onSelected = jest.fn((t: EligibleTerminal) => t.name);
      const logger = createMockLogger();

      const result = await showTerminalPicker(
        items,
        quickPickProvider,
        createHandlers(onSelected, { onDismissed: () => 'dismissed-via-non-bindable' }),
        logger,
      );

      expect(result).toBe('dismissed-via-non-bindable');
      expect(onSelected).not.toHaveBeenCalled();
    });
  });
});
