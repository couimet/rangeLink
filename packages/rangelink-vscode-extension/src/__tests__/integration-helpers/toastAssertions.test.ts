import {
  assertNoToastLogged,
  assertToastLogged,
} from '../../__integration-tests__/helpers/toastAssertions';

describe('toastAssertions', () => {
  describe('assertToastLogged', () => {
    ...
    it('passes when matching status bar message is found', () => {
      const lines = [
        '[DEBUG] {"fn":"VscodeAdapter.setStatusBarMessage","message":"✓ RangeLink bound to Terminal","timeout":2000} Setting status bar message',
      ];

      assertToastLogged(lines, {
        type: 'statusBar',
        message: '✓ RangeLink bound to Terminal',
      });
    });
    ...
    it('throws when toast type matches but message differs', () => {
      ...
      expect(() =>
        assertToastLogged(lines, { type: 'info', message: 'expected message' }),
      ).toThrow('Expected info toast with message "expected message"');
    });
    ...
    it('throws when message matches but toast type differs', () => {
      ...
      expect(() =>
        assertToastLogged(lines, { type: 'info', message: 'some message' }),
      ).toThrow('Expected info toast');
    });

    it('throws when no log lines provided', () => {
      expect(() =>
        assertToastLogged([], { type: 'info', message: 'anything' }),
      ).toThrow('not found in 0 log lines');
    });
    ...
    it('finds match among multiple log lines', () => {
      const lines = [
        ...
        '[DEBUG] {"fn":"VscodeAdapter.setStatusBarMessage","message":"✓ Bound","timeout":2000} Setting status bar message',
      ];

      assertToastLogged(lines, { type: 'statusBar', message: '✓ Bound' });
    });
  });
  ...
  describe('assertNoToastLogged', () => {
    ...
    it('throws when matching toast is found', () => {
      ...
      expect(() =>
        assertNoToastLogged(lines, { type: 'warning', message: 'found it' }),
      ).toThrow('Expected warning toast with message "found it" to NOT be logged');
    });
  });
});
