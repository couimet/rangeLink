 ...
 *
 * Only callable when `process.env.RANGELINK_TEST_FIXTURES_ENABLED === 'true'`.
...
export const markRangeLinkTestFixture = (terminal: vscode.Terminal): void => {
  if (process.env.RANGELINK_TEST_FIXTURES_ENABLED !== 'true') {
 ...
 * `RANGELINK_TEST_FIXTURES_ENABLED` is not `'true'`, so production code pays
 * only a single env-var read.
 */
export const isRangeLinkTestFixture = (terminal: vscode.Terminal): boolean => {
  if (process.env.RANGELINK_TEST_FIXTURES_ENABLED !== 'true') return false;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
