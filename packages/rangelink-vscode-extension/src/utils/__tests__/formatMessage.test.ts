...
describe('formatMessage', () => {
  ...
  describe('Remaining Hardcoded Messages (Issue #37)', () => {
    beforeEach(() => {
      supportedLocales.en = messagesEn;
      setLocale('en');
    });

    it('should format ERROR_INVALID_DELIMITER_CONFIG (static)', () => {
      const result = formatMessage(MessageCode.ERROR_INVALID_DELIMITER_CONFIG);
    ...
    it('should format ERROR_NO_ACTIVE_EDITOR (static)', () => {
      const result = formatMessage(MessageCode.ERROR_NO_ACTIVE_EDITOR);

      expect(result).toStrictEqual('RangeLink: No active editor');
    });

    it('should format ERROR_NO_TEXT_SELECTED (static)', () => {
      const result = formatMessage(MessageCode.ERROR_NO_TEXT_SELECTED);

      expect(result).toStrictEqual('RangeLink: No text selected. Select text and try again.');
    });

    it('should format ERROR_VERSION_INFO_NOT_AVAILABLE (static)', () => {
      const result = formatMessage(MessageCode.ERROR_VERSION_INFO_NOT_AVAILABLE);

      expect(result).toStrictEqual('Version information not available');
    });

    it('should format INFO_VERSION_COPY_COMMIT_HASH_BUTTON (static)', () => {
      const result = formatMessage(MessageCode.INFO_VERSION_COPY_COMMIT_HASH_BUTTON);

      expect(result).toStrictEqual('Copy Commit Hash');
    });

    it('should format INFO_VERSION_INFO with all params', () => {
      const result = formatMessage(MessageCode.INFO_VERSION_INFO, {
        version: '1.2.3',
        commit: 'abc1234',
        isDirtyIndicator: ' (with uncommitted changes)',
        branch: 'main',
        buildDate: '2024-01-15T10:30:00Z',
      });

      expect(result).toStrictEqual(
        'RangeLink v1.2.3\nCommit: abc1234 (with uncommitted changes)\nBranch: main\nBuild: 2024-01-15T10:30:00Z',
      );
    });

    it('should format INFO_VERSION_INFO with empty isDirtyIndicator', () => {
      const result = formatMessage(MessageCode.INFO_VERSION_INFO, {
        version: '1.0.0',
        commit: 'def5678',
        isDirtyIndicator: '',
        branch: 'feature/test',
        buildDate: '2024-01-10T08:00:00Z',
      });
