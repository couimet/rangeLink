...
- - **Context menu items hidden for non-file editors** - "Send File Path", "Send Relative File Path", and "Send RangeLink" family commands no longer appear in the context menus of output panels, Settings UI, search editors, and other non-file surfaces. Previously these commands had no scheme guard and would route meaningless identifiers (output channel IDs, `"settingseditor"`) to the bound destination. (#509)
